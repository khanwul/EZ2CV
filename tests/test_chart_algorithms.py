...
from ez2cv.chart.pipeline import (_apply_timing_uncertainty, ChartNote,
                                  _convert_and_snap_notes,
                                  _predicted_max_combo)
...
class ChartAlgorithmTest(unittest.TestCase):
    ...
    def test_short_longnote_tail_never_collapses_to_tap(self):
        clock = TickClock.from_drafts(
            [BPMDraft(0.0, 2_000.0, 120.0, 120.0)], origin_ms=0.0)
        raw = SimpleNamespace(notes=[RawNote(
            0, "longnote", 0.0, 31.25, "white", 1.0,
            end_sigma_ms=1.0)])

        notes, _, _, diagnostics = _convert_and_snap_notes(
            raw, clock, [0, 768], 768)

        self.assertEqual(notes[0].end_tick, 12)
        self.assertTrue(notes[0].needs_review)
        self.assertEqual(diagnostics[0]["tail_snap_source"], "raw-fallback")

    def test_tail_bias_cannot_promote_a_measure_to_a_finer_grid(self):
        clock = TickClock.from_drafts(
            [BPMDraft(0.0, 2_000.0, 120.0, 120.0)], origin_ms=0.0)
        raw = SimpleNamespace(notes=[
            RawNote(0, "longnote", clock.tick_to_ms(0),
                    clock.tick_to_ms(16), "white", 1.0),
            RawNote(1, "longnote", clock.tick_to_ms(48),
                    clock.tick_to_ms(80), "white", 1.0),
        ])

        notes, _, levels, _ = _convert_and_snap_notes(
            raw, clock, [0, 768], 768)

        self.assertEqual(levels, [0])
        self.assertTrue(all(note.end_tick is not None for note in notes))

    def test_tail_calibration_never_moves_release_earlier(self):
        clock = TickClock.from_drafts(
            [BPMDraft(0.0, 2_000.0, 120.0, 120.0)], origin_ms=0.0)
        raw = SimpleNamespace(notes=[RawNote(
            lane, "longnote", clock.tick_to_ms(lane * 48),
            clock.tick_to_ms(25 + lane * 48), "white", 1.0)
            for lane in range(3)])

        _, _, _, diagnostics = _convert_and_snap_notes(
            raw, clock, [0, 768], 768)

        self.assertEqual({d["tail_bias_tick"] for d in diagnostics}, {0.0})

    def test_tail_calibration_refits_a_shared_early_phase(self):
        clock = TickClock.from_drafts(
            [BPMDraft(0.0, 2_000.0, 120.0, 120.0)], origin_ms=0.0)
        heads = (0, 48, 96, 144)
        releases = (96, 144, 192, 240)
        raw = SimpleNamespace(notes=[RawNote(
            lane, "longnote", clock.tick_to_ms(head),
            clock.tick_to_ms(release - 3), "white", 1.0)
            for lane, (head, release) in enumerate(zip(heads, releases))])

        notes, _, _, diagnostics = _convert_and_snap_notes(
            raw, clock, [0, 768], 768)

        self.assertEqual([note.end_tick for note in notes], list(releases))
        self.assertEqual({d["tail_bias_tick"] for d in diagnostics}, {-3.0})
    ...
    def test_repeated_fine_heads_set_a_chart_wide_grid_floor(self):
        ...
        _, _, levels, _ = _convert_and_snap_notes(
            raw, clock, [0, 192, 384, 576, 768, 960], 768)

        self.assertTrue(all(level >= 2 for level in levels))

    def test_longnote_partial_combo_interval_rounds_up(self):
        notes = [ChartNote(0, 0, None), ChartNote(1, 0, 64)]
        self.assertEqual(_predicted_max_combo(notes), 3)
