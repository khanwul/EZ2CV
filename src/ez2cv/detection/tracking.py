...
class NoteTracker:
    ...
    def _tail_lag_frames(self, speed: float | None = None) -> float:
        ...
        """
        sp = self.speed.speed if speed is None else speed
        ...
        for i in range(len(traj) - 1):
            ...
            if ya < self.trigger <= yb and yb != ya:
                ...
                if fb - fa > self.max_stale and e.type == "lnhead" and sp > 0:
                    local = self._local_crossing(traj[:i + 1], self.trigger)
                    if local is None:
                        projection = (self.trigger - ya) / sp
                        cf = fa + projection
                    ...
                    cf = fa + frac * (fb - fa)
                    local = self._local_crossing(traj[:i + 2], self.trigger)
                    if local is not None:
                        _, sp, local_sigma_ms = local
                        sigma_ms = max(sigma_ms, local_sigma_ms)
                if e.type == "lntail":
                    cf += self._tail_lag_frames(sp)
                match = (sa + sb) / 2
    ...
    def _extrapolate_trigger(self, e):
        ...
        if yb >= self.trigger:
            return None
        global_speed = self.speed.speed
        if (global_speed <= 0
                or (self.trigger - yb) > 3 * global_speed):
            return None
        local = (None if e.type == "lnhead"
                 else self._local_crossing(e.trajectory, self.trigger))
        if local is None:
            sp = global_speed
            projection = (self.trigger - yb) / sp
            cf = fb + projection
            sigma_ms = self._fallback_sigma_ms(projection)
        else:
            cf, sp, sigma_ms = local
        if e.type == "lntail":
            cf += self._tail_lag_frames(sp)
        proj_ratio = (self.trigger - yb) / (3.0 * global_speed)
