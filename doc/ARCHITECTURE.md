...
nor `Chart` depends on detection configuration objects. `write_chart()` serializes it as
`ez2cv.chart` 3.3: `meta.game` identifies the game, timing lives under one
`timing` object, fixed and ramped BPM segments declare their interpolation,
meter changes are tick-addressed events, and derived statistics and diagnostics
live under `analysis`.
